...
怎么检验你的地图画得够不够好？做一个"冷启动测试"：开一个全新的 agent 会话，只让它看仓库内容，看它能不能回答五个基本问题。
...
**原则 3：最小但完备。** 每条知识都应该有明确的使用场景。如果你删掉某条规则不影响 agent 的决策质量，那这条规则就不应该存在。但冷启动测试中的每个问题都必须有答案。这是一个需要持续调整的平衡，不多不少，刚好够用。
